...
class ObjectDaoService(private val db: OrientDatabase) {
    ...
    fun getTruncatedObjects(pattern: String = "") =
        logTime(logger, "request of all truncated objects") {
            transaction(db) {
                ...
                if (pattern.isBlank()) {
                    return@transaction getAllObjectsTruncated(tsById)
                } else {
                    return@transaction searchObjectsTruncated(pattern, tsById)
                }
            }
        }
    ...
    private fun anyOfCond(conds: List<String>) = conds.joinToString(" or ", "(", ")")
    ...
    private fun searchObjectsTruncated(pattern: String, tsById: Map<String, Instant>): List<ObjectTruncated> =
        searchInNameAndDescription(pattern, tsById) + searchInStringValues(pattern, tsById) + searchInReferenceBook(pattern, tsById)
    ...
    private fun searchInNameAndDescription(pattern: String, tsById: Map<String, Instant>): List<ObjectTruncated> {
        ...
        return db.query(query, mapOf("lq" to luceneQuery(textOrAllWildcard(pattern)))) { it.map { extractObject(it, tsById) } }.toList()
    }
    ...
    private fun searchInStringValues(pattern: String, tsById: Map<String, Instant>): List<ObjectTruncated> {
        val queryByStr = baseSelectForTruncatedObjects +
                ...
                " and (deleted is NULL or deleted = false ) "
        return db.query(queryByStr, mapOf("lq" to luceneQuery(textOrAllWildcard(pattern)))) { it.map { extractObject(it, tsById) } }.toList()
    }
    ...
    private fun searchInReferenceBook(pattern: String, tsById: Map<String, Instant>): List<ObjectTruncated> {
        val queryByRefBook = baseSelectForTruncatedObjects +
                ...
                " and (deleted is NULL or deleted = false ) "
        return db.query(queryByRefBook, mapOf("lq" to luceneQuery(textOrAllWildcard(pattern)))) { it.map { extractObject(it, tsById) } }.toList()
    }

    private fun getAllObjectsTruncated(tsById: Map<String, Instant>): List<ObjectTruncated> {
        val query =
            "SELECT @rid, name, description, " +
                    "FIRST(OUT(\"${OrientEdge.GUID_OF_OBJECT.extName}\")).guid as guid, " +
                    "FIRST(OUT(\"$OBJECT_SUBJECT_EDGE\")).name as subjectName, " +
        ...
        return db.query(query) { it.map { extractObject(it, tsById) } }.toList()
    }
