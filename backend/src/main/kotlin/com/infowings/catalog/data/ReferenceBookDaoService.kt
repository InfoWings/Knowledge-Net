...
class ReferenceBookDaoService(private val db: OrientDatabase) {
    fun newReferenceBookItemVertex(): ReferenceBookItemVertex =
        db.createNewVertex(REFERENCE_BOOK_ITEM_VERTEX).toReferenceBookItemVertex()

    fun newReferenceBookVertex(): ReferenceBookVertex =
        db.createNewVertex(REFERENCE_BOOK_VERTEX).toReferenceBookVertex()
